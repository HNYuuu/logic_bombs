...
| ----------------------------- | ----------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------- |
| Symbolic Variable Declaration | stdin_svd.c             | user inputs 7                                                         | Compiled                                                      |
|                               | cpu_svd.c               | the program runs on an Intel CPU                                      | Compiled                                                      |
|                               | time_svd.c              | current time is after Jan 1st, 2050                                   | Compiled                                                      |
|                               | pid_svd.c               | the process id is 4096                                                | Fail (hard encode 42 for pid by compiler)                     |
|                               | vm_svd.c                | the program runs on an virtual machine                                | Compiled                                                      |
|                               | web_svd.c               | if a remote website contains the string "trigger the bomb"            | Compiled                                                      |
|                               | syscall_ls_svd.c        | the number of files under a current directory is 7                    | Compiled                                                      |
| Contextual Symbolic Value     | file_csv.c              | if argv[1] points to a file                                           | Compiled                                                      |
|                               | pid_csv.c               | if argv[1][0]-48 equals the process id%78                             | Compiled                                                      |
|                               | ping_csv.c              | if argv[1] points to a live IP                                        | Compiled                                                      |
|                               | syscall_csv.c           | if argv[1] is a valid Linux command                                   | Compiled                                                      |
| Covert Propagation            | file_cp.c               | data propagate via a file (expected argv[1][0]: '7')                  | Compiled                                                      |
|                               | stack_cp.c              | data propagation via direct push/pop (expected argv[1][0]: '7')       | Inapplicable (Compiler cannot compile)                        |
|                               | echo_cp.c               | data propagation via echo (expected argv[1][0]: '7')                  | Compiled                                                      |
|                               | df2cf_cp.c              | data propagation via control flow (expected argv[1][0]: '7')          | Pass                                                          |
|                               | echofile_cp.c           | data propagation via echo and file (expected argv[1][0]: '7')         | Compiled                                                      |
|                               | socket_cp.c             | data propagation via socket (expected argv[1][0]: '7')                | Cannot compiled                                               |
|                               | toy_eh_cp.cpp           | data propagation via exception handling (expected argv[1][0]: '7')    | Compiled                                                      |
|                               | div0_eh_cp.cpp          | raise an exceptions when divided by 0 (expected argv[1][0]: '7')      | Compiled                                                      |
|                               | file_eh_cp.cpp          | expected argv[1]: an existed file                                     | Compiled                                                      |
...
| Symbolic Jump                 | funcpointers_sj_l1.c    | call a function related to stdin (expected argv[1][0]: '5')           | TODO                                                          |
...
| Crypto Function               | sha_cf.c                | if sha1(i) equals to a predefined value, expected argv[1][0] = '7'    | Compiled                                                      |
|                               | aes_cf.c                | if aes(i, plaintext) equals to a ciphertext                           | Compiled                                                      |
| Loop                          | collaz_lo_l1.c          | if it loops 25 times (example argv[1][0]: '7')                        | Compiled                                                      |
|                               | 5n+1_lo_l1.c            | if it loops 25 times (example argv[1][0]: '7')                        | Compiled                                                      |
|                               | 7n+1_lo_l1.c            | if it loops 50 times (example argv[1][0]: '7')                        | Compiled                                                      |
|                               | collaz_lo_l2.c          | if it loops 986 times (example argv[1][0]: '7')                       | Compiled                                                      |
|                               | paraloop_lo_l2.c        | a loop terminated by another thread (expected argv[1][0]: '7')        | Compiled                                                      |
